...
'''
#------------------------------------------
# 8. БЛОК ЧИСЛЕННЫХ ЭКСПЕРИМЕНТОВ (РАСШИРЕННЫЙ)
#------------------------------------------
# Этот блок полностью автономен. Он добавляет в сайдбар две кнопки
# для последовательного запуска большого набора готовых сценариев.
def analyze_phase_portrait(signal, delay=1):
    """
    Анализ фазового портрета для определения типа динамики
...
def plot_and_analyze_phase_portrait(time_series, title="", delays=[1, 2], include_3d=True):
    ...
    if is_multidimensional:
        ...
        with col1:
            fig, analysis = plot_phase_portrait(time_series, delays, title, plot_3d=include_3d)
            st.plotly_chart(fig)
...
def get_default_range(param_name):
    """Возвращает диапазон по умолчанию для заданного параметра"""
    if param_name == 'r' or param_name == 'Темп роста популяции':
        return (0.1, 3.5)
    elif param_name == 'tau' or param_name == 'Временная задержка':
        return (1, 10)
    elif param_name == 'K' or param_name == 'Предельная численность среды':
        return (10, 200)
    elif param_name == 'sigma' or param_name == 'Уровень случайных колебаний':
        return (0.01, 0.5)
    elif param_name == 'fertility' or param_name == 'Коэффициент рождаемости':
        return (0.1, 2.0)
    elif param_name == 'survival' or param_name == 'Вероятность выживания':
        return (0.1, 1.0)
    elif 'delay' in param_name or 'Задержка' in param_name:
        return (0, 5)
    elif 'migration' in param_name or 'Скорость миграции' in param_name:
        return (0.0, 0.5)
    else:
        return (0.1, 2.0)


def generate_bifurcation_for_experiment(scenario):
    """
    Генерирует и анализирует бифуркационную диаграмму для параметра, 
    который варьируется в эксперименте
    """
    # Определяем модель и параметры
    run_function = scenario.get('run_function')
    params = scenario.get('params', [])
    plot_type = scenario.get('plot_type', 'multiple')
    
    # Для типа 'comparison' бифуркационный анализ не подходит
    if plot_type == 'comparison':
        st.info("Бифуркационный анализ недоступен для сравнения разных моделей")
        return
    
    # Для фазовых портретов тоже пропускаем
    if plot_type == 'phase_portraits':
        st.info("Бифуркационный анализ недоступен для демонстрации фазовых портретов")
        return
    
    if not params:
        st.warning("Нет параметров для анализа")
        return
    
    # Находим варьируемый параметр, сравнивая наборы параметров
    varied_param = None
    ...
    if(True):
        if len(params) >= 2:
            ...
            # Удаляем метки и специальные ключи
            for p in [p1, p2]:
                if 'label' in p: p.pop('label', None)
                if 'features' in p: p.pop('features', None)
                if 'func' in p: p.pop('func', None)
                if 'base_sim' in p: p.pop('base_sim', None)
                if 'args' in p: p.pop('args', None)
            
            # Ищем отличающийся параметр
            for key in p1:
                if key in p2 and p1[key] != p2[key]:
                    varied_param = key
                    break
        
        # Если не удалось найти или нашли неподходящий параметр, используем эвристику
        if varied_param is None or varied_param == "model_type":
            if len(params) > 0:
                p = params[0].copy()
                if 'label' in p: p.pop('label', None)
                
                # Для стохастической модели приоритет - sigma и r
                if run_function == simulate_stochastic:
                    if 'sigma' in p:
                        varied_param = 'sigma'
                    else:
                        varied_param = 'r'  # Предполагаем, что r всегда есть
                
                # Для модели с задержкой приоритет - tau
                elif run_function == simulate_delay and 'tau' in p:
                    varied_param = 'tau'
                
                # Для гибридной модели приоритеты
                elif run_function == simulate_hybrid:
                    priority_params = ['r', 'delay_fert', 'delay_surv', 'env_effect', 'stoch_intensity']
                    for param in priority_params:
                        if param in p:
                            varied_param = param
                            break
                
                # Для моделей Лесли приоритеты
                elif run_function == simulate_leslie:
                    if 'fertility' in p:
                        varied_param = 'fertility'
                    elif 'survival' in p:
                        varied_param = 'survival'
                
                # Для остальных моделей приоритет - r
                elif 'r' in p:
                    varied_param = 'r'
                
                # Если всё ещё не найден, берем первый параметр
                if varied_param is None or varied_param == "model_type":
                    # Исключаем служебные параметры
                    excluded_keys = ['label', 'features', 'func', 'base_sim', 'args', 'model_type']
                    for key in p:
        ...
        if varied_param is None or varied_param == "model_type":
            st.warning("Не удалось определить подходящий параметр для бифуркационного анализа")
            return
    # Определяем диапазон значений для бифуркации
    param_values = []
    for p in params:
        if varied_param in p:
            # Проверяем, является ли параметр списком
            if isinstance(p[varied_param], list):
                # Если это список, берем первое значение для упрощения
                # Это может быть случай с fertility, survival, migration_rates и т.д.
                param_values.append(p[varied_param][0] if p[varied_param] else 0)
            else:
                param_values.append(p[varied_param])
        elif varied_param == 'r' and 'args' in p and len(p['args']) >= 2:
            param_values.append(p['args'][1])  # Извлекаем r из args для стохастической модели

    # Создаем диапазон с запасом
    if len(param_values) >= 2:
        # Убедимся, что все значения числовые
        numeric_values = [float(val) for val in param_values if isinstance(val, (int, float))]
        if numeric_values:
            min_val = min(numeric_values)
            max_val = max(numeric_values)
            range_width = max_val - min_val
            
            # Если диапазон очень мал, расширяем его
            if range_width < 0.1:
                range_width = 0.5
                
            # Убеждаемся, что минимальное значение не отрицательное
            param_range = (max(0, min_val - range_width * 0.3), max_val + range_width * 0.5)
        else:
            # Параметр-специфичные диапазоны по умолчанию
            param_range = get_default_range(varied_param)
    else:
        # Параметр-специфичные диапазоны по умолчанию
        param_range = get_default_range(varied_param)
    
    # Настройки бифуркации
    bifurcation_steps = 50
    bifurcation_discard = 50
    
        # Получаем T из первого параметра
    T = None
    for p in params:
        # Прямой поиск T в параметрах
        if 'T' in p:
            T = p['T']
            break
        
        # Поиск в args, если args есть
        if 'args' in p:
            args = p['args']
            
            # Если args - словарь
            if isinstance(args, dict) and 'T' in args:
                T = args['T']
                break
            
            # Если args - кортеж или список
            elif isinstance(args, (list, tuple)):
                # Проверяем длину, чтобы избежать IndexError
                if len(args) >= 4:
                    T = args[3]  # T обычно 4-й аргумент
                    break
            
            # Если args что-то другое, возможно, это уже сам T
            elif isinstance(args, (int, float)):
                T = args
                break
        
        # Дополнительная проверка для стохастической модели, 
        # где T может быть частью первого аргумента
        if 'base_sim' in p and 'args' in p and isinstance(p['args'], tuple) and len(p['args']) >= 4:
            T = p['args'][3]
            break

    # Если T не найден, используем значение по умолчанию        
    if T is None:
        T = 100  # Значение по умолчанию
    
    # Подготавливаем базовые параметры
    base_params = params[0].copy()
    base_params.pop('label', None)
    
    # Обработка специальных случаев для получения корректных base_params
    if plot_type == 'comparison':
        # Для сравнения моделей берем параметры первой модели
        if 'func' in base_params:
            run_function = base_params.pop('func')
        if 'args' in base_params:
            args_dict = base_params.pop('args')
            if isinstance(args_dict, dict):
                base_params.update(args_dict)
            elif isinstance(args_dict, tuple):
                # Преобразуем позиционные аргументы в именованные
                if run_function == simulate_logistic or run_function == simulate_ricker:
                    if len(args_dict) >= 4:
                        base_params['N0'] = args_dict[0]
                        base_params['r'] = args_dict[1]
                        base_params['K'] = args_dict[2]
                        base_params['T'] = args_dict[3]
                elif run_function == simulate_delay:
                    if len(args_dict) >= 5:
                        base_params['N0'] = args_dict[0]
                        base_params['r'] = args_dict[1]
                        base_params['K'] = args_dict[2]
                        base_params['T'] = args_dict[3]
                        base_params['tau'] = args_dict[4]
    
    # Специальная обработка для разных типов моделей
    if run_function == simulate_stochastic:
        base_sim = base_params.pop('base_sim', simulate_logistic)
        args = base_params.pop('args', (10, 1.0, 100, T))
        
        # Конвертируем args в словарь
        args_dict = {}
        ...
        with st.spinner("Построение бифуркационной диаграммы..."):
            fig_bifurcation, df_bifurcation = bifurcation_diagram_stochastic(
                base_sim=base_sim,
                param_name=varied_param,
                param_range=param_range,
                steps=bifurcation_steps,
                T_sim=T,
                discard=bifurcation_discard,
                repeats=min(base_params.get('repeats', 20), 20),  # Ограничиваем для скорости
                sigma=base_params.get('sigma', 0.1),
                current_params={**base_params, **args_dict}
            )
    else:
        with st.spinner("Построение бифуркационной диаграммы..."):
            # Специальная обработка для моделей с векторными параметрами
            if run_function == simulate_hybrid:
                # Убеждаемся, что все необходимые параметры есть
                if 'N0_vec' not in base_params and 'T' not in base_params:
                    st.warning("Недостаточно параметров для бифуркационного анализа гибридной модели")
                    return
                
                # Инициализируем векторные параметры нулями
                if varied_param == "migration_rates":
                    base_params["migration_rates"] = [0.0] * len(base_params.get('N0_vec', []))
                elif varied_param == "delay_fert":
                    base_params["delay_fert"] = [0] * len(base_params.get('N0_vec', []))
                elif varied_param == "delay_surv":
                    base_params["delay_surv"] = [0] * (len(base_params.get('N0_vec', [])) - 1)
            
            elif run_function == simulate_leslie:
                # Убеждаемся, что все необходимые параметры есть
                if 'N0_vec' not in base_params or 'fertility' not in base_params or 'survival' not in base_params:
                    st.warning("Недостаточно параметров для бифуркационного анализа модели Лесли")
                    return
                
                # Инициализируем векторные параметры нулями
                if varied_param == "fertility":
                    base_params["fertility"] = [0.0] * len(base_params.get('N0_vec', []))
                elif varied_param == "survival":
                    base_params["survival"] = [0.0] * (len(base_params.get('N0_vec', [])) - 1)
            
            fig_bifurcation, df_bifurcation = bifurcation_diagram(
                model_func=run_function,
                param_name=varied_param,
                param_range=param_range,
                steps=bifurcation_steps,
                T_sim=T,
    ...
    st.subheader(f"Бифуркационная диаграмма для параметра '{varied_param}'")
    st.plotly_chart(fig_bifurcation)
    
    # Автоматический анализ бифуркационной диаграммы
    if(True):
        st.subheader("Анализ бифуркационной диаграммы")
        # Группируем данные по значению параметра и анализируем
        grouped = df_bifurcation.groupby(varied_param)
        counts = grouped['N'].count()
        variance = grouped['N'].var()
        mean = grouped['N'].mean()
        
        # Определяем области разных режимов
        regime_thresholds = {}
        
        # 1. Ищем переход от стационарного к периодическому режиму (первая бифуркация)
        # Используем дисперсию как индикатор - резкий рост дисперсии указывает на бифуркацию
        variance_threshold = variance.mean() * 0.2
        periodic_regions = variance[variance > variance_threshold].index.tolist()
        
        # Поиск первой точки бифуркации
        first_bifurcation = None
        if len(periodic_regions) > 0 and len(variance) > 5:
            # Сглаживаем ряд дисперсии для более надежного определения
            smoothed_var = variance.rolling(window=3, min_periods=1).mean()
            
            # Ищем точку, где дисперсия начинает существенно расти
            for i in range(2, len(smoothed_var)):
                if smoothed_var.iloc[i] > 3 * smoothed_var.iloc[i-2]:
                    first_bifurcation = smoothed_var.index[i]
                    break
        
        # 2. Определяем регионы с разным поведением
        regions = []
        
        # Используем подсчет уникальных значений для определения периодичности
        unique_counts = []
        for param_val in grouped.groups.keys():
            values = df_bifurcation[df_bifurcation[varied_param] == param_val]['N'].values
            
            # Округляем до 3 знаков после запятой для более надежного определения периодичности
            rounded_values = np.round(values, 3)
            unique_values = np.unique(rounded_values)
            unique_counts.append((param_val, len(unique_values)))
        
        unique_df = pd.DataFrame(unique_counts, columns=[varied_param, 'unique_count']).sort_values(by=varied_param)
        
        # Определяем периодичность в каждой области
        current_period = 1
        period_changes = []
        
        for i in range(1, len(unique_df)):
            prev_count = unique_df.iloc[i-1]['unique_count']
            curr_count = unique_df.iloc[i]['unique_count']
            
            if curr_count > prev_count * 1.5 and curr_count > 3:  # Значительное увеличение
                period_changes.append((unique_df.iloc[i][varied_param], curr_count))
                
                # Для каскада удвоения периода
                if curr_count == prev_count * 2:
                    current_period *= 2
        
        # Выводим результаты анализа
        st.markdown(f"""
        ## Автоматический анализ бифуркационной диаграммы для параметра '{varied_param}'
        
        ### Основные характеристики:
        - **Диапазон параметра:** {param_range[0]:.3f} - {param_range[1]:.3f}
        - **Количество точек:** {bifurcation_steps}
        - **Отброшены первые шаги:** {bifurcation_discard}
        """)
        
        # Анализ первой бифуркации
        if first_bifurcation is not None:
            st.markdown(f"""
            ### Критические точки:
            - **Первая бифуркация:** {first_bifurcation:.3f}
              - При этом значении система переходит от стационарного режима к колебательному
            """)
            
            # Добавляем информацию о последующих бифуркациях
            if len(period_changes) > 0:
                st.markdown("### Последующие бифуркации:")
                for i, (param_val, period) in enumerate(period_changes):
                    st.markdown(f"- **Бифуркация {i+2}:** {param_val:.3f} (переход к периоду ~{period})")
        
        # Интерпретация диаграммы в зависимости от модели
        ...
        if run_function == simulate_logistic or run_function == simulate_ricker:
            if varied_param == 'r':
                ...
                **Параметр r (темп роста)** является ключевым для определения динамики:
                
                - **Малые значения r**: стационарное состояние - популяция стабильна
                - **Средние значения r**: колебательный режим - популяция осциллирует с периодом 2, затем 4, 8 и т.д.
                - **Высокие значения r**: хаотический режим - непредсказуемые колебания
                
                Эта последовательность бифуркаций известна как **каскад удвоения периода** и является классическим путем к хаосу.
                """
            elif varied_param == 'K':
                model_interpretation = """
                ### Интерпретация:
                
                **Параметр K (емкость среды)** обычно не вызывает качественных изменений динамики, 
                но влияет на абсолютные значения:
                
                - При малых K и высоком r может наблюдаться дестабилизация из-за усиления эффектов самоограничения
                - Увеличение K обычно стабилизирует популяцию
                """
            ...
            if varied_param == 'tau':
                ...
                **Параметр τ (задержка)** определяет, насколько система "запаздывает" в своей реакции:
                
                - **Малые значения τ**: близко к обычной логистической модели
                - **Критическое значение τ**: система теряет устойчивость через бифуркацию Хопфа, появляются колебания
                - **Большие значения τ**: сложные колебания, возможен хаос
                
                Задержка всегда действует как **дестабилизирующий фактор**, так как система реагирует на устаревшую информацию.
                """
            ...
            if varied_param == 'fertility':
                ...
                **Параметр fertility (рождаемость)** определяет способность популяции к росту:
                
                - **Малые значения**: популяция вымирает
                - **Критическое значение**: стабильная популяция (собственное значение матрицы Лесли = 1)
                - **Высокие значения**: экспоненциальный рост
                
                В отличие от других моделей, модель Лесли редко показывает колебательные режимы при изменении только рождаемости,
                поскольку изменяется в основном скорость роста, а не характер динамики.
                """
            elif varied_param == 'survival':
                model_interpretation = """
                ### Интерпретация:
                
                **Параметр survival (выживаемость)** определяет структуру популяции:
                
                - **Малые значения**: быстрое вымирание
                - **Средние значения**: стабильная возрастная структура
                - **Высокие значения**: смещение к старшим возрастным группам
                
                Выживаемость влияет не только на общую численность, но и на соотношение возрастных групп.
                """
                
        elif run_function == simulate_stochastic:
            if varied_param == 'sigma':
                model_interpretation = """
                ### Интерпретация:
                
                **Параметр σ (интенсивность шума)** определяет стохастичность системы:
                
                - **Малые значения σ**: близко к детерминированной модели
                - **Средние значения σ**: случайные отклонения вокруг детерминированной траектории
                - **Высокие значения σ**: полностью стохастическое поведение, возможны случайные вымирания
                
                На бифуркационной диаграмме стохастичность проявляется как "размытие" структуры.
                Острые границы между режимами заменяются плавными переходами.
                """
            ...
            if varied_param == 'r':
                ...
                **Параметр r (базовый темп роста)** в гибридной модели взаимодействует с другими механизмами:
                
                - Задержки усиливают эффект от изменения r
                - Возрастная структура может как усиливать, так и сглаживать влияние r
                - Миграция между группами обычно стабилизирует систему
                
                Гибридная модель часто демонстрирует более сложные бифуркации из-за взаимодействия множества факторов.
                """
            elif 'delay' in varied_param:
                model_interpretation = """
                ### Интерпретация:
                
                **Параметры задержки** в гибридной модели:
                
                - Даже небольшие задержки могут дестабилизировать ранее устойчивую систему
                - Задержки в разных процессах (рождаемость, выживаемость) могут взаимно усиливать друг друга
                - Критические значения задержек зависят от других параметров (r, миграция)
                
                Задержки часто являются главным источником колебаний в сложных биологических системах.
                """
                
        # Выводим интерпретацию в зависимости от модели
        if model_interpretation:
            st.markdown(model_interpretation)
        
        # Визуализация дисперсии для идентификации бифуркаций
        st.subheader("Дисперсия состояний как индикатор бифуркаций")
        variance_df = pd.DataFrame({
            varied_param: variance.index,
            'Дисперсия': variance.values
        })
        fig_var = px.line(variance_df, x=varied_param, y='Дисперсия', 
                          title=f"Зависимость дисперсии от параметра {varied_param}")
        st.plotly_chart(fig_var)
        
        st.markdown("""
        ### Как читать график дисперсии:
        - **Резкий рост дисперсии** указывает на бифуркацию
        - **Постепенное увеличение** может свидетельствовать о приближении к хаотическому режиму
        - **Локальные пики** могут указывать на структурные изменения в системе
        """)
        '''
