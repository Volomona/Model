...
if model == "Модель с задержкой":
    ...
    # Используем общую начальную численность для распределения по группам
    with st.sidebar.expander("Начальная численность"):
        st.markdown(f"""
            <div style="color: #666; font-size:0.9rem; margin-bottom:10px;">
            Сколько особей в каждой группе в начале моделирования (всего {common['N0']})
            </div>
            """, unsafe_allow_html=True)
        
        # Предлагаем равномерное распределение общей начальной численности
        default_per_group = common['N0'] / n
        ...
        N0_vec = [
            st.number_input(
                ...
                f"🔢 Группа {i + 1} (старые)",
                min_value=0.0,
                value=default_per_group  # Используем часть общей численности
            ) for i in range(n)
        ]
        if st.button("Равномерно распределить численность"):
            N0_vec = [common['N0']/n] * n
            st.rerun()
        # Показываем предупреждение, если сумма не равна общей численности
        total_initial = sum(N0_vec)
        if abs(total_initial - common['N0']) > 0.01:
            st.warning(f"Общая начальная численность по группам ({total_initial:.1f}) отличается от заданной общей численности ({common['N0']})")
